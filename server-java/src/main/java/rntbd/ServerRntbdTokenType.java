...
public enum ServerRntbdTokenType {
    ...
    private static final ServerRntbdTokenType[] allTokens = getAllTokens();

    private static  ServerRntbdTokenType[] getAllTokens() {
        final int maxByteValue = 0xFF + 1;
        final ServerRntbdTokenType[] allPossibleTokens = new ServerRntbdTokenType[maxByteValue]; // one byte RNTBD limit
        for(int i = 0; i < maxByteValue; i++) {
            allPossibleTokens[i] = Invalid;
        }

        // Override with valid entries
        for (final ServerRntbdTokenType tokenType : ServerRntbdTokenType.values()) {
    ...
    ServerRntbdTokenType(final byte id, final Codec codec) {
        this.codec = codec;
        this.id = id;
    }
    ...
    public static ServerRntbdTokenType fromId(final byte value) {
        if (value == Invalid.id) {
            return Invalid;
        }
