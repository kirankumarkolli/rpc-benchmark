...
namespace Microsoft.Azure.Documents
{
    ...
    internal abstract class RntbdTokenStream<T>
        where T : Enum
    {
        private static int[] TokenPositionMap;
        ...
        private static int[] GetTokenPositions(RntbdToken[] t)
        {
            ...
            {
                TokenPositionMap[t[i].GetTokenIdentifier()] = i;
            }
        ...
        public void ParseFrom(ref BytesDeserializer reader)
        {
            ...
            while(reader.Position < reader.Length)
            {
                ...
                RntbdToken token = this.tokens[TokenPositionMap[identifier]];
                if (token == null)
                {
                ...
                if (token.isPresent)
                {
                    Trace.TraceError("Duplicate token with identifier {0} type {1} found in RNTBD token stream",
                ...
                switch (token.GetTokenType())
                {
                    ...
                    default:
                        Trace.TraceError("Unrecognized token type {0} with identifier {1} found in RNTBD token stream",
